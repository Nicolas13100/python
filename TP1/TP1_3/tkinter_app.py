...
def saluer_utilisateurs_depuis_fichier(fichier_noms, fichier_messages):
    try:
        ...
        if not os.path.exists(fichier_noms):
            messagebox.showerror("Erreur", f"Le fichier {fichier_noms} est introuvable. Veuillez vérifier le chemin du fichier.")
            return
...
def choisir_fichier_noms():
    chemin_fichier_noms = filedialog.askopenfilename(title="Choisissez le fichier contenant les noms", filetypes=[("Text Files", "*.txt")])
...
def choisir_fichier_messages():
    chemin_fichier_messages = filedialog.askopenfilename(title="Choisissez le fichier contenant les messages", filetypes=[("Text Files", "*.txt")])
